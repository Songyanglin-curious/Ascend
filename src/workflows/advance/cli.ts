...
export const EXPLICIT_EXIT_REMINDER = "已结束当前节点本轮推进。";
...
export function isAwaitIntentFillerInput(rawInput: string): boolean {
  ...
  return /^(嗯|哦|啊|呃|emmm|uh|。|\.\.+|[?!？！,，。]+)$/.test(normalized);
}
...
export function appendFixedAssistantMessage(state: AgentState, message: string): AgentState {
  return {
    ...state,
...
export async function processAdvanceInput(
  ...
  if (isExplicitExitInput(rawInput)) {
    ...
    const exitState: AgentState = {
      ...appendFixedAssistantMessage(stateWithUser, EXPLICIT_EXIT_REMINDER),
      shouldExit: true,
      phase: "ended",
    };
    ...
    return {
      state: exitState,
      assistantOutput: EXPLICIT_EXIT_REMINDER,
      handoffRecord: buildHandoffRecord(exitState, "explicit_exit"),
    };
  }

  if (state.phase === "await_c_intent" && isAwaitIntentFillerInput(rawInput)) {
    const reminderState = appendFixedAssistantMessage(state, AWAIT_INTENT_FILLER_REMINDER);
  ...
  if (isEmptyNormalizedQuery(nextState.normalizedQuery)) {
    const reminderState = appendFixedAssistantMessage(nextState, EMPTY_INPUT_REMINDER);
...
export async function runAdvanceCli(
  ...
  try {
    ...
    while (true) {
      const prompt = state.phase === "await_c_intent" ? "confirm/reject> " : "input> ";
      ...
      if (result.handoffRecord) {
        ...
        return buildAdvanceSceneRawResult(state, result.handoffRecord.endReason);
      }
    }
  } finally {
    await io.close?.();
